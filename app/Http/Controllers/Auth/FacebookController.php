   ...
   public function __construct()
    ...
    protected function loginOrCreateAccount($providerUser, $driver)
    {
        ...
        if( $user ) {
            ...
            if($user->user_sid==""){
                 try{
                        $user1 = $twilio->chat->v2->services($setting->TWILIO_CHAT_SERVICE_SID)
                                      ->users
                                      ->create($providerUser->getEmail());
                        $user->user_sid=$user1->sid;
                        $user->save();
                 }catch(Exception $e){
            ...
            if($user->user_sid!=""){
                if($user->profile_pic!=""){
                    $image="profile/".$user->profile_pic;
                }else{
                    $image="profile/profile.png";
                }
                $array=array("id"=>$user->id,"name"=>$providerUser->getName(),"profile"=>$image);
                $user1 = $twilio->chat->v2->services($setting->TWILIO_CHAT_SERVICE_SID)
                         ->users($user->user_sid)
                         ->update([
